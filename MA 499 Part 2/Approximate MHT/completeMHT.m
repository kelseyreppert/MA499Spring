...
% MHT Variables
m = 2*ones(1,scans+1); % The number of measurements at each scan k

iniT = 2; % Number of initial targets
CT = linspace(1, iniT, iniT); % Confirmed targets

[OMEGA, TT] = generateHyp(m(1), CT); % The set of all hypotheseses
...
for k = 1:scans
    ...
    % Update KF Variables
    PHI = [1 0 -dt*speed*sin(x(3,k)); 0 1 dt*speed*cos(x(3,k)); 0 0 1 - dt*speed/Rad];
    ...
    for i = 1:Nhyp
        ...
        DT = Ndt(i);
        for j = 1:DT
            product = product*mvnpdf(z(:,k+1), H*xbar(:,k+1), B(:,:,k+1)); %%% NOTE: this should be z_j?
        end
